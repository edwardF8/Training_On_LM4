...
# =============================================================================
# Template lists — one per field, hoisted to module scope so both the legacy
# full-bio renderer (get_text_simple3) and the field-configurable renderer
# (render_bio) can reference the same paraphrase pools.
# =============================================================================
...
def get_text_simple3(person, exposure, master_seed=0, order=None, reverse_md=False):
    """Generate one paraphrased bioS biography for `person` (all 6 fields).

    Deterministic in (master_seed, person['id'], exposure):
    calling with the same triple always returns the same string.

    Kept for backwards compatibility with the original Capo pipeline. New code
    that wants to control which fields appear should use `render_bio` instead.
    """
    rng_seed = (master_seed * 1_000_003 + person["id"]) * 1_000_003 + exposure
    rng = random.Random(rng_seed)

    sentence1 = " " + rng.choice(TEMPLATES_BIRTHDAY)
    sentence2 = " " + rng.choice(TEMPLATES_BIRTHCITY)
    sentence3 = " " + rng.choice(TEMPLATES_UNIVERSITY)
    sentence4 = " " + rng.choice(TEMPLATES_FIELD)
    sentence5 = " " + rng.choice(TEMPLATES_COMPANY_CITY)
    sentence6 = " " + rng.choice(TEMPLATES_COMPANY_NAME)
...
# =============================================================================
# Field-configurable bio rendering
# =============================================================================
#
# Each entry in FIELD_SPECS describes one attribute the bio can include:
#   - templates:           paraphrase pool (module-level constants above).
#   - value_placeholder:   the `{...}` slot name those templates use for the
#                          field's value (e.g. "birthday", "birthcity").
#   - render_value(p):     produce the value string from a person dict.
#   - subject:             "name" if the template's `{name}` slot takes the
#                          full name; "pronoun" if it takes He/She.
#
# render_bio(person, exposure_idx, fields) emits exactly the requested fields
# in the requested order. For each field, template_idx = exposure_idx % T
# (round-robin), so K exposures per person give every paraphrase at least
# floor(K/T) coverage when K >= T — useful so eval prompts never use a
# template the training data didn't include.

FIELD_SPECS = {
    "birthday": {
        "templates":         TEMPLATES_BIRTHDAY,
        "value_placeholder": "birthday",
        "render_value":      lambda p: f"{p['birthmonth']} {p['birthday']}, {p['birthyear']}",
        "subject":           "name",
    },
    "birthcity": {
        "templates":         TEMPLATES_BIRTHCITY,
        "value_placeholder": "birthcity",
        "render_value":      lambda p: p["birthcity"],
        "subject":           "pronoun",
    },
    "university": {
        "templates":         TEMPLATES_UNIVERSITY,
        "value_placeholder": "university",
        "render_value":      lambda p: p["university"],
        "subject":           "pronoun",
    },
    "field": {
        "templates":         TEMPLATES_FIELD,
        "value_placeholder": "field",
        "render_value":      lambda p: p["field"],
        "subject":           "pronoun",
    },
    "company_city": {
        "templates":         TEMPLATES_COMPANY_CITY,
        "value_placeholder": "company1city",
        "render_value":      lambda p: p["company1city"],
        "subject":           "pronoun",
    },
    "company_name": {
        "templates":         TEMPLATES_COMPANY_NAME,
        "value_placeholder": "company1name",
        "render_value":      lambda p: p["company1name"],
        "subject":           "pronoun",
    },
}


def render_bio(person, exposure_idx, fields, reverse_md=False):
    """Render a bio containing only the specified fields.

    Deterministic in (person["id"], exposure_idx, fields). For each enabled
    field f, template_idx = exposure_idx % len(FIELD_SPECS[f]["templates"]).
    """
    name = f"{person['first_name']} {person['middle_name']} {person['last_name']}"
    ...
    for field in fields:
        ...
        template = templates[exposure_idx % len(templates)]
        subject = name if spec["subject"] == "name" else pronoun

        if field == "birthday" and reverse_md:
            value = (f"{person['birthday']} of "
                     f"{person['birthmonth']}, {person['birthyear']}")
        else:
...
def bio_stream(people, K=100, master_seed=0, shuffle_seed=1, fields=None):
    """Yield (person_idx, exposure_idx, bio_text) tuples in shuffled order.

    If `fields` is None, emit the legacy 6-field bio (get_text_simple3).
    Otherwise emit only the requested fields via render_bio.
    """
