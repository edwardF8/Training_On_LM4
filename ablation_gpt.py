"""Ablation sweep over Capo bioS models — GPT2(RoPE) variant.

Mirror of ablation.py with CONFIG.MODEL_TYPE flipped to "gpt2", so the same
(layers, heads, epochs) sweeps run on the standard-MLP backbone from
create_gpt2_model instead of the gated-MLP Llama backbone. Cache and runs/
directories are namespaced separately so the two ablations coexist cleanly.
"""
...
CONFIG.NAME = "bioS_name_date_small_gpt_abalation"
...
SWEEP_NAME = f"bioS_name_date_small_gpt_abalation-{INVOCATION}"
